...
bool Raster::rasterTriangles(Picture* picture, ZBuffer* zbuffer, const std::vector<trimesh::vec3>& triangles, const trimesh::box3& aabb, RasterConfig* rasterConfig)
{
    if (!picture || !zbuffer || triangles.empty() || !rasterConfig)
    {
        return false;
    }
